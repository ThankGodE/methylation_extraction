...
def add_package2env_var() -> None:
    ...
    path_2_run_scrit_base_dir = run_script_base_dir()

    path2env_var = find_env_var_file(path_2_run_scrit_base_dir)
...
def load_package_paths(env_var_files: str) -> None:
    ...
    if isinstance(env_var_files, list) is True:
        [specifilly_load_files(env_file) for env_file in env_var_files]

    elif isinstance(env_var_files, str) is True:
        specifilly_load_files(env_var_files)


def specifilly_load_files(file2load: str) -> None:
